        ...
        <div className="h-screen w-full">
            ...
            <main className="w-full h-full pt-[115px] bg-[#FFFFFF] p-6">
                <form onSubmit={handleSubmit(submit)} className="bg-[#DEDEDE] font-bold text-[28px] shadow-md rounded-md p-8 mt-5">
                    ...
                    <h1 className={twMerge('text-center font-kanit sm:text-left text-slate-800 mb-[3rem] font-medium text-[36px] uppercase')} >Cadastro de Acontecimento</h1>
                            ...
                            {ativacao &&
                                <ul className="absolute translate-y-[6px]">
                                    {status_acontecimentos.map((acontecimento, index) => 
                                        <li><button onClick={(e) => {e.preventDefault(); setStatus_acontecimento(index); setAtivacao(!ativacao)}} className="w-[300px] h-11 text-[16px] font-normal rounded-md text-slate-100 hover:text-[#ffffff] bg-[#353941] hover:bg-[#4a4e57] active:border-2">{acontecimento}</button></li>
                                    )}
                                </ul>}
                        </div>
                    </div>
