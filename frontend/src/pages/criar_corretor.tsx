...
export function CriarCorretor () { 
    ...
    const imobiliarias = ["fdasfsdf", "João", "Daniel"]; // Puxar do Backend
                ...
                <form onSubmit={handleSubmit(submit)} className="bg-[#DEDEDE] font-bold text-[28px] shadow-md rounded-md p-8 mt-5">
                    ...
                    <div className="grid grid-cols-4 items-center gap-32">
                        <div className="col-span-2">
                            <label htmlFor="nome_imovel" className="text-[18px] text-slate-700 font-outfit">Nome do Corretor</label>
                        ...
                        <div className="">
                            ...
                            <button onClick={(e) => {e.preventDefault(); setAtivacao(!ativacao)}} {...register('id_imobiliaria')} value={status_imobiliaria} className="w-[300px] h-12 text-[16px] rounded-md bg-[#353941] hover:bg-[#4a4e57] active:border-2 flex justify-between items-center px-5">
                                <h6 className="text-slate-100 hover:text-[#ffffff] font-normal">{imobiliarias[status_imobiliaria]}</h6>
                            ...
                            {ativacao &&
                                <ul className="absolute translate-y-[6px]">
                                    {imobiliarias.map((imobiliaria, index) => 
                                        <li><button onClick={(e) => {e.preventDefault(); setStatus_imobiliaria(index); setAtivacao(!ativacao)}} className="w-[300px] h-11 text-[16px] font-normal rounded-md text-slate-100 hover:text-[#ffffff] bg-[#353941] hover:bg-[#4a4e57] active:border-2">{imobiliaria}</button></li>
                                    )}
                                </ul>}
                        </div>
                    </div>
