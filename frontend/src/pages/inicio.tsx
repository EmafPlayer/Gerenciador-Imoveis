      ...
      <body className="relative h-full w-full pl-16 z-0 pt-[95px]">
        ...
        <div className="relative z-10">
          <div className="flex items-center justify-between w-[50%]">
            ...
            <div>
              <button onClick={() => redirect(user[0])} className="bg-green-500 p-2 text-[#ffffff]"><IoAdd/></button>
            </div>
          </div>
          <div className="grid grid-rows-3 gap-5 w-[50%]">
          {casas[contador].map((casa) => 
            <div className="bg-[#ffffff] h-[13rem] w-full p-5 hover:bg-slate-50 rounded-xl shadow-md border-2 border-[#a1a1a1d3] border-3 flex items-center justify-between">
              <div className="flex">
                <img src={`../../public/${casa.foto}`} alt="Foto da casa" className="h-[165px] w-[250px] rounded-lg"/>
                <div className="pl-8 pt-4">
                  <h1 className="text-[22px] text-slate-700 pb-2">{casa.nome}</h1>
                  <h1 className="text-[18px] text-slate-600">{casa.endereço}</h1>
                </div>
              </div>
            </div>
          )}
