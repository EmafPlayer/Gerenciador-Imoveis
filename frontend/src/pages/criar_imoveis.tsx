...
export function CriarImoveis () {
    ...
    const tipo = ["Aluguel", "Venda", "Aluguel e Venda"];
    ...
    const submit = async (data: any) => 
    {
        try {
            const params = new URLSearchParams({
                ...
                anunciado: anunciado ? String(1) : String(0),
            }).toString();
                    ...
                    <form onSubmit={handleSubmit(submit)} className="bg-[#DEDEDE] font-bold text-[28px] shadow-md rounded-md p-8 mt-5 mb-12">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 lg:gap-32 items-end">
                            ...
                            <div className="col-span-1 flex items-center gap-3 pb-5">
                                <label htmlFor="anunciado" className="text-[18px] text-slate-700 font-outfit">Foi anunciado?</label>
                                <input type="checkbox" name="anunciado" id="anunciado" checked={anunciado} onChange={(e) => setAnunciado(e.target.checked)} className="accent-[#353941] w-6 h-6 rounded-lg transition duration-300 delay-300 ease-in-out"/>
                            </div>
